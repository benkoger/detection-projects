...
try:
    ...
    print("Please install osgeo to use all functions in 'mapping_functions.py'.")
...
def from_image_to_map(uv, z_guess, pmatrix_dict, pix4d, max_guesses, 
                      correct_threshold, object_height=1, verbose=True):
    ...
    if not found_point:
        if verbose:
            print("couldn't find precise point", orig_z_guess, z_guess, z_diff)
...
def get_pix4d_info(map_folder, observation_name, pmatrix_sort='ungulates',
    ...
    dsm_gtif = rasterio.open(geotif_dsm_file)
    
    # Get information for converting between utm and raster coordinates
    dsm_transform = dsm_gtif.transform
    pix4d_info['x_origin'] = dsm_transform[2]
    pix4d_info['y_origin'] = dsm_transform[5]
    pix4d_info['pixel_width'] = dsm_transform[0]
    pix4d_info['pixel_height'] = dsm_transform[4]
    
    geotif_dtm_file = os.path.join(map_folder, "3_dsm_ortho", "extras", "dtm", 
                                   f"{observation_name}_dtm.tif")             
    dtm_gtif = rasterio.open(geotif_dtm_file)
    dtm = dtm_gtif.read(1)
